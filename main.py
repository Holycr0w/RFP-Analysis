...
class Analyzer:
    ...
    def analyze_gap(context: str) -> str:
        try:
            ...
            You are a specialized RFP compliance auditor with expertise in identifying gaps between requirements and responses. 
            
            Analyze the following RFP requirements and Response content with systematic precision:
            ...
            1. COMPLIANCE SUMMARY:
               ...
               - Assessment of proposal viability based solely on compliance
               - Top 3 compliance strengths
               - Top 3 compliance vulnerabilities

            2. REQUIREMENTS GAP MATRIX:
               For each substantive RFP requirement:
               - Requirement: [Exact text from RFP]
               - Status: [COMPLIANT, NON-COMPLIANT, or PARTIALLY COMPLIANT]
               - Evidence: [Specific section/text from Response that addresses this]
               - Gap Description: [Clear explanation of exactly what's missing]
               - Impact: [HIGH, MEDIUM, LOW] with justification
               - Remediation Complexity: [HIGH, MEDIUM, LOW]
               - Priority: [CRITICAL, IMPORTANT, DESIRABLE]

            3. FATAL FLAWS:
               Identify any disqualifying gaps that would automatically render the proposal non-compliant
               - For each fatal flaw:
                  * Requirement section reference
                  * Missing element
                  * Verification that it's truly absent (not just in a different section)
                  * Remediation steps with urgency indicator

            4. RED FLAGS:
               Identify areas where the Response uses vague language, makes unsupportable claims, or provides 
               generic content instead of specific compliance statements
               - For each red flag:
                  * Current text/approach
                  * Problem categorization (Vague, Unsupported, Generic, etc.)
                  * Proposed resolution approach

            5. PRIORITIZED REMEDIATION PLAN:
               - Categorize gaps by effort required and impact on compliance
               - Provide actionable recommendations in priority order
               - For each recommendation:
                  * Target section
                  * Specific addition/modification
                  * How this directly addresses an RFP requirement

            Be decisive in your assessments. Avoid hedging language. Use precise terminology and provide 
            specific evidence for each finding. Focus on identifying exactly what must be changed to 
            achieve full compliance.
            """

            output = llm.invoke(gap_analysis_prompt)
    ...
    def generate_insights(context: str) -> str:
        try:
            llm = get_llm()
            insight_prompt = f"""
            You are a specialized RFP analysis expert with years of experience in government and enterprise procurement. Your task is to perform a precise, detailed comparison between an RFP and its corresponding Response. DO NOT MAKE ANY ASSUMPTIONS or Tell any false information . Focus solely on the content provided.

            Based on the following documents:

            {context}

            Generate a comprehensive analysis using this structured framework:

            1. EXECUTIVE SUMMARY (250 words maximum):
               - Clear identification of whether this Response is likely to be successful
               - Primary strengths and critical deficiencies
               - Overall confidence score (0-100%) regarding Response's alignment with RFP

            2. REQUIREMENTS ANALYSIS (Be specific and exhaustive):
               - For each distinct requirement in the RFP:
                  * Requirement ID: [Extract or assign an identifier]
                  * Requirement Description: [Brief description]
                  * Status: [Choose ONE: FULLY ADDRESSED, NOT ADDRESSED, or INADEQUATELY ADDRESSED]
                  * Evidence: [Exact quotes or section references from the Response]
                  * Confidence: [HIGH, MEDIUM, LOW] with reasoning
                  * Gap Details: [If not FULLY ADDRESSED, explain specifically what's missing]
               - Never use ambiguous terminology. Be definitive in your assessment.

            3. COMPETITIVE ADVANTAGES (Identify 3-5):
               - List specific areas where the Response exceeds RFP requirements
               - For each advantage, explain why it provides value beyond the requirements

            4. CRITICAL VULNERABILITIES (Identify 3-5):
               - List specific requirements that are completely missing or severely underaddressed
               - For each vulnerability, estimate potential impact on proposal success (HIGH/MEDIUM/LOW)
               - Suggest precise, actionable remediation for each vulnerability

            5. DETAILED IMPROVEMENT RECOMMENDATIONS:
               - Provide 5-7 specific, actionable recommendations
               - For each recommendation:
                  * Target section in the Response
                  * Current text/approach
                  * Suggested revision or addition
                  * Rationale tied directly to RFP requirements

            Throughout your analysis:
            - Make definitive statements rather than hedging
            - Use precise language with high information density
            - Cite specific sections and page numbers whenever possible
            - Focus on objective evaluation rather than subjective assessment
            - Prioritize identification of critical gaps that could disqualify the proposal
            """
...
class ReportFormatter:
    ...
    def format_report(raw_data: str) -> str:
        try:
            ...
            ## Part 1: Gap Analysis
            {raw_data.split('## Part 1:')[1].split('## Part 2:')[0] if '## Part 1:' in raw_data else raw_data.split('COMPLIANCE SUMMARY')[0]}

            ## Part 2: Detailed Insights
            {raw_data.split('## Part 2:')[1] if '## Part 2:' in raw_data else raw_data.split('EXECUTIVE SUMMARY')[0]}
            """
